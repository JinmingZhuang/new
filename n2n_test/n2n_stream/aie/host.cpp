...
#define W1 16
#define W2 2
#define DATA_SIZE1 H1*W1
#define DATA_SIZE2 W1*W2
#define OUT_SIZE H1*W2
...
int main(int argc, char** argv) {
    ...
    int sizeIn1 = DATA_SIZE1;
    int sizeIn2 = DATA_SIZE2;
    int sizeOut = OUT_SIZE;

    uint16_t DataInput0[sizeIn1], DataInput1[sizeIn2];
    uint16_t pre[H1][W2];
    uint16_t golden[sizeOut];
    ...
    xrtBufferHandle in_bohdl0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(uint16_t), 0, 0);
    auto in_bomapped0 = reinterpret_cast<uint16_t*>(xrtBOMap(in_bohdl0));
    memcpy(in_bomapped0, DataInput0, sizeIn1 * sizeof(uint16_t));
    printf("Input memory virtual addr 0x%px\n", in_bomapped0);

    xrtBufferHandle in_bohdl1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(uint16_t), 0, 0);
    auto in_bomapped1 = reinterpret_cast<uint16_t*>(xrtBOMap(in_bohdl1));
    memcpy(in_bomapped1, DataInput1, sizeIn2 * sizeof(uint16_t));
    printf("Input memory virtual addr 0x%px\n", in_bomapped1);
    ...
    xrtBufferHandle out_bohdl = xrtBOAlloc(dhdl, sizeOut * sizeof(uint16_t), 0, 0);
    auto out_bomapped = reinterpret_cast<uint16_t*>(xrtBOMap(out_bohdl));
    memset(out_bomapped, 0xABCDEF00, sizeOut * sizeof(uint16_t));
    ...
    xrtRunHandle mm2s_rhdl1 = xrtKernelRun(mm2s_khdl1, in_bohdl0, nullptr, sizeIn1);
    printf("run pl_mm2s_1\n");

    xrtKernelHandle mm2s_khdl2 = xrtPLKernelOpen(dhdl, top->m_header.uuid, "pl_mm2s:{pl_mm2s_2}");
    xrtRunHandle mm2s_rhdl2 = xrtKernelRun(mm2s_khdl2, in_bohdl1, nullptr, sizeIn2);
    ...
    for(int r1=0; r1<H1; r1++){
        for(int c2=0; c2<W2; c2++){
            for(int c1=0; c1<W1; c1++){
                pre[r1][c2]=DataInput0[c1*H1+r1]*DataInput1[c1+c2*W1]+pre[r1][c2];
            }
        }
